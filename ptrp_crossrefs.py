"""In this version we reset patterns for every iteration."""
...
parser.add_argument("--overlap", default=0, type=float, dest='overlap',
    help="Fractional overlap of adjacent string sections (every sections reaches out to the left over\
          the regular boundary by int(overlap*string_len) bits. (default: %(default)s)")

parser.add_argument("--num-refs", default=0, type=int, dest='num_refs',
    help="Number of uniformly drawn node pairs that are connected by a cross-rerefence. (default: %(default)s)")
...
def make_tree(lam,N,gamma,overlap,num_refs):
    G,root=utils.poisson_ditree(lam)
    leaves = utils.leaves(G)
    #r=len(list(G.successors(root)))
    #N_eff=r*N
    walker=rw.SRPWalker(G.copy(),root,N,gamma,overlap,num_refs)
    walker.set_weights()
...
G,root,walker=make_tree(lam,N,gamma,overlap,num_refs)

...
scratch = '/home/k1801311/Documents/KCL/PhD/codes/pattern_walker/outputs/animations'
data_dir = os.path.join(scratch, project_name)
...
for job in jobs:
    ...
    with open(job_file,'w') as fh:
        ...
        fh.writelines("python animation.py \
            --lam {r} --seed {seed} --gamma {gamma} --string-len {N} --overlap {overlap} --job-name {job_name} --output-dir {out_dir}\
             \n".format(out_dir=job_name_data,**job))

    subprocess.run(job_file,shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)

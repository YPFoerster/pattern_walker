...
def spanning_tree_with_root(G,root,edge_direction='up'):
    ...
    if edge_direction=='up':
        ...
        return None
def tree_weight(G,root,edge_direction='up',weight_str='weight'):
    ...
    for (u,v) in tree.edges:
        temp*=G.get_edge_data(u,v)[weight_str]
    return temp
...
def tree_mfpts(tree,start,target,weight_str='weight'):
    #Find MFPT based on tree weights. Tree has to be an arborescence.
    sigma_target=tree_weight(tree,target,weight_str=weight_str)
    ...
    for cut_node_ndx in range(len(shortest_path)-1):
        ...
        temp_sigma*=tree_weight(target_comp,target,weight_str=weight_str)
        temp_sigma*=np.sum([tree_weight(start_comp,node,weight_str=weight_str) for node in start_comp])
